...
while true; do
  read -r -s -p "管理员密码（至少8位）: " ADMIN_PASS
  echo ""
  if [[ ${#ADMIN_PASS} -lt 8 ]]; then
    echo "密码长度不足8位，请重试" >&2
    continue
  fi
  read -r -s -p "再次输入管理员密码: " ADMIN_PASS2
  echo ""
...
echo "[3/7] 下载并安装程序 (${PACKAGE_NAME})..."
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "${TMP_DIR}"' EXIT
echo "[3/7] 下载地址: ${PACKAGE_URL}"
echo "[3/7] 保存到: ${TMP_DIR}/${PACKAGE_NAME}"
curl -fL "${PACKAGE_URL}" -o "${TMP_DIR}/${PACKAGE_NAME}"
tar -xzf "${TMP_DIR}/${PACKAGE_NAME}" -C "${TMP_DIR}"
if [[ ! -f "${TMP_DIR}/supervisor-panel" ]]; then
  echo "安装包缺少 supervisor-panel 二进制文件: ${PACKAGE_URL}" >&2
  exit 1
fi
...
cat > "${ENV_FILE}" <<EOF
SP_ADDR=:${PANEL_PORT}
SP_DB_PATH=${DATA_DIR}/supervisor-panel.db
...
SP_CADDY_ENABLED=true
SP_CADDY_DATA_DIR=${DATA_DIR}/caddy
SP_UPDATE_SCRIPT_PATH=${INSTALL_DIR}/upgrade.sh
EOF
chmod 600 "${ENV_FILE}"

echo "[5/7] 初始化管理员..."
"${BIN_PATH}" init-admin --db "${DATA_DIR}/supervisor-panel.db" --username "${ADMIN_USER}" --password "${ADMIN_PASS}"
...
systemctl enable supervisor-panel
systemctl restart supervisor-panel

echo "[7/7] 安装完成"
IP_ADDR="$(hostname -I | cut -d ' ' -f 1)"
echo "访问地址: http://${IP_ADDR:-127.0.0.1}:${PANEL_PORT}"
echo "管理员用户名: ${ADMIN_USER}"
echo "安装包来源: ${PACKAGE_URL}"
echo "安装成功。"
